...
function init() {
  ...
  sounds = {
    ...
    hadoken: new Audio('sounds/hadouken.wav'),
...
function updateEntities(dt, gameTime) {
  ...
  else if (level.scrolling && player.pos[0] < vX + 15) {
    vX = player.pos[0] - 16;
  }
  ...
  hadokens.forEach(function(hadoken) {
    hadoken.update(dt);
  });
  level.pipes.forEach (function(pipe) {
    pipe.update(dt);
  });
  level.door.forEach (function(door) {
    door.update(dt);
  });
}
...
function render() {
  ...
  for(var i = 0; i < 15; i++) {
    for (var j = Math.floor(vX / 16) - 1; j < Math.floor(vX / 16) + 35; j++){
